...
- (void)sendFeedback{
    if ([self isExistenceNetwork]) {
        ASIFormDataRequest * request = [ASIFormDataRequest requestWithURL:[NSURL URLWithString:@"http://apis.iyuba.com/v2/api.iyuba?protocol=91001"]];
        request.delegate = self;
        [request setPostValue:@"xml" forKey:@"format"];
        NSInteger nowUserId = [[[NSUserDefaults standardUserDefaults] objectForKey:@"nowUser"] integerValue];
        NSString * decodedText = [[NSString alloc] initWithString:[_feedback.text URLEncodedString]];
        if (nowUserId > 0) {
            [request setPostValue:[NSString stringWithFormat:@"%d",nowUserId] forKey:@"uid"];
        }
        else
        {
            [request setPostValue:[NSString stringWithFormat:@"0"] forKey:@"uid"];
