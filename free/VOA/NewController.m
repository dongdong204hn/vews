...
//    self.title = kNewThree;
    ...
    [titleBtn setTitle:@"最新" forState:UIControlStateNormal];
    category = 0;
    nowTitle = kClassAll;
//    [self setMytitleUp];
...
- (void)catchIntroduction:(NSInteger)maxid pages:(NSInteger)pages pageNum:(NSInteger)pageNumOne{
    NSString *url = [NSString stringWithFormat:@"http://voa.iyuba.com/voa/titleWordstoryApi.jsp?maxid=%d&type=iOS&format=xml&pages=%d&pageNum=%d",maxid,pages,pageNumOne];
...
- (void)requestSoundStarted:(ASIHTTPRequest *)request
{
    ...
    //    NSString *url = [NSString stringWithFormat:@"http://voa.iyuba.com/voa/textWordstoryApi.jsp?voaid=%d&format=xml",request.tag];
    //    //    NSLog(@"catch:%d",voaid._voaid);
    //    //    ASIHTTPRequest * request = [[ASIHTTPRequest alloc] initWithURL:[NSURL URLWithString:url]];
